...
impl From<DepartmentWrapper> for Department {
  fn from(value: DepartmentWrapper) -> Self {
    match value {
      DepartmentWrapper::SDK => Department::SDK,
      DepartmentWrapper::DJIGO => Department::DJIGO,
      DepartmentWrapper::DJIFly => Department::DJIFly,
      DepartmentWrapper::AgriculturalMachinery => Department::AgriculturalMachinery,
      DepartmentWrapper::Terra => Department::Terra,
      DepartmentWrapper::DJIGlasses => Department::DJIGlasses,
      DepartmentWrapper::DJIPilot => Department::DJIPilot,
      DepartmentWrapper::GSPro => Department::GSPro,
      DepartmentWrapper::Unknown => Department::Unknown(u8::MAX),
    }
  }
}
...
impl DJILogWrapper {
  ...
  pub fn fetch_keychains(
    &self,
    api_key: String,
    department_wrapper: DepartmentWrapper,
  ) -> Result<Vec<Vec<KeychainFeaturePointWrapper>>, DJIError> {
    use dji_log_parser::layout::auxiliary::Department;
    let department = Department::from(department_wrapper);

    let request = self
      .inner
      .keychains_request_with_custom_params(Option::Some(department), None);

    let request = request.map_err(|_| DJIError::KeychainError)?;
    let chains = request
      .fetch(&api_key, None)
      .map_err(|_| DJIError::KeychainError)?;
